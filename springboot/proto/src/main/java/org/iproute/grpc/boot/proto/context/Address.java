...
public class Address {
    ...
    public static Address from(SocketAddress socketAddress, boolean local) {
        return Objects.isNull(socketAddress) ? unknown(local) :
                ((socketAddress instanceof InetSocketAddress addr)
                        ? Address.builder().host(addr.getHostString()).port(addr.getPort()).build() : unknown(local)
                );
